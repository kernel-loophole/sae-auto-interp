...
class TensorBuffer:
    ...
    def __iter__(self):
        ...
        if self.features is None:
            self.features = torch.unique(self.locations[:, 2])  # Find unique features
        return self
    ...
    def __next__(self):
        ...
        # Skip if there are fewer than the minimum examples for the feature
...
class FeatureDataset:
    """
    Dataset that constructs TensorBuffers for each module and feature.
    """
    def __init__(
        self,
        raw_dir: str,
    ...
    def _build_selected(self, raw_dir: str, modules: List[str], features: Dict[str, int]):
        """
        Build dataset buffers which load only selected features.
        """
        edges = self._edges()
